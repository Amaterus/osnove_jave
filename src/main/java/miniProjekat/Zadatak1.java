...
//        Napomena: Posto je procenat realan broj, u stampi ce se prikazati vise cifara iza zareza.
        ...
        int numPackets = (fileSize + 4) / 5;

        String fileContent = "";
        int downloadedBytes = 0;

        for (int i = 1; i <= numPackets; i++) {
            System.out.print("Unesite paket " + i + ": ");
            String packet = s.nextLine();
            fileContent = fileContent + packet;

            downloadedBytes = downloadedBytes + packet.length();
            double downloadedPercentage = ((double) downloadedBytes / fileSize) * 100.0;

            if (downloadedPercentage > 100.0) {
                downloadedPercentage = 100.0;
            }

            System.out.println("Skinuto " + downloadedPercentage + "%");

            if (downloadedPercentage >= 100.0) {
                break;
            }
        }
        System.out.println("Sadrzaj fajla je: " + fileContent);
    }
}
